...
export const updateCategory = asyncHandler(async (req, res) => {
  ...
  const category = await Category.findByIdAndUpdate(req.params.id, data, { returnDocument: 'after', runValidators: true }); if (!category) return res.status(404).json({ message: 'Category not found.' }); clearPublicCache(); revalidateHomepage().catch(err => console.error('Revalidation failed:', err)); res.json({ category })
})
